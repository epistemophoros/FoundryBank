...
export class FoundryBank {
  ...
  private static bankerSystem: BankerSystem;
  ...
  static async init(): Promise<void> {
    ...
    (this.bankManager as any).economySystem = this.economySystem;
  ...
  private static registerHooks(): void {
    ...
    Hooks.on('clickToken', (token: Token, event: MouseEvent) => {
    ...
    // Hook into token HUD
    Hooks.on('renderTokenHUD', (hud: TokenHUD, html: JQuery, token: Token) => {
    ...
    Hooks.on('renderActorSheet', (sheet: any, html: JQuery, data: any) => {
    ...
    Hooks.on('foundrybank.calculateInterest', async () => {
      const interestRate = game.settings.get('foundrybank', 'enableInterest') 
        ? (game.settings.get('foundrybank', 'interestRate') as number || 0.02)
        : 0;
   ...
   */
  private static registerTokenControls(): void {
   ...
   * Add GM Manager button to settings menu
   */
  private static addGMManagerButton(): void {
   ...
   */
  private static addTokenHUDButton(hud: TokenHUD, html: JQuery, token: Token): void {
    const bankButton = $(`
    ...
    bankButton.on('click', (event: JQuery.ClickEvent) => {
      event.preventDefault();
      event.stopPropagation();
      this.openBankDialog(token);
    });
   ...
   */
  private static addBankerHUDButton(hud: TokenHUD, html: JQuery, token: Token): void {
    const bankerButton = $(`
    ...
    bankerButton.on('click', (event: JQuery.ClickEvent) => {
      event.preventDefault();
      event.stopPropagation();
      this.openBankDialogFromBanker(token);
    });
   ...
   */
  private static addBankerSheetButton(sheet: any, html: JQuery): void {
  ...
  static openGMManagerDialog(): void {
    ...
    const dialog = new GMManagerDialog(
      this.economySystem,
      this.bankManager,
...
Hooks.once('ready', () => {
  if (game.modules.get('foundrybank')?.active) {
    game.foundrybank = {
      ...
      openBankDialog: (token: Token) => FoundryBank.openBankDialog(token),
      openBankDialogFromBanker: (token: Token) => FoundryBank.openBankDialogFromBanker(token)
    };
  }
});
