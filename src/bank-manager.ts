...
export interface Bank {
  id: string;
  name: string;
  economyId: string; // Bank belongs to an economy
  description: string;
  location?: string;
  createdAt: number;
  isActive: boolean;
...
export interface BankAccount {
  id: string;
  bankId: string; // Account belongs to a bank
  actorId: string;
  accountName: string;
  balance: number;
  currencyId: string; // Uses currency from economy system
  createdAt: number;
  lastTransaction: number;
  lastInterestCalculation: number; // When interest was last calculated
  interestEarned: number; // Total interest earned
  syncWithActorCurrency: boolean; // Sync with D&D 5e currency (optional)
}
...
export interface Transaction {
  ...
  amount: number;
  currencyId: string; // Uses currency ID from economy system
  description: string;
  timestamp: number;
  fromAccountId?: string;
  toAccountId?: string;
  exchangeRate?: number; // For currency exchanges
  exchangedAmount?: number; // Amount after exchange
}
...
export class BankManager {
  ...
  private interestCalculationInterval?: number;
  private economySystem?: any; // Will be set after EconomySystem is initialized
   ...
   */
  private startInterestCalculation(): void {
   ...
   */
  async calculateAllInterest(interestRate?: number): Promise<void> {
    if (interestRate === undefined) {
      // Get from settings or use default
      interestRate = game.settings.get('foundrybank', 'interestRate') as number || 0.02;
    }
  ...
  createAccount(
    ...
    accountName: string,
    currencyId: string,
    syncWithActorCurrency: boolean = false
  ): BankAccount {
   ...
   */
  async transfer(fromAccountId: string, toAccountId: string, amount: number, description: string = ''): Promise<Transaction> {
   ...
   */
  getAccountTransactions(accountId: string, limit: number = 50): Transaction[] {
   ...
   */
  getActorTransactions(actorId: string, limit: number = 50): Transaction[] {
    return Array.from(this.transactions.values())
  ...
  syncActorCurrency(actor: Actor): void {
    ...
    const currency = (actor.system as any)?.currency;
    if (!currency) return;
   ...
   */
  private mapCurrencyToDnd5e(currencyId: string): string | null {
   ...
   */
  async depositFromActorCurrency(accountId: string, amount: number): Promise<Transaction> {
    const account = this.accounts.get(accountId);
   ...
   */
  async withdrawToActorCurrency(accountId: string, amount: number): Promise<Transaction> {
    const account = this.accounts.get(accountId);
    if (!account || !account.syncWithActorCurrency) {
      throw new Error('Account does not sync with actor currency');
    }
   ...
   */
  async calculateInterest(accountId: string, interestRate: number): Promise<number> {
    const account = this.accounts.get(accountId);
    if (!account) {
      throw new Error(`Account ${accountId} not found`);
    }
    ...
    if (daysSinceLastInterest < 1) return 0; // Only calculate daily
  ...
  private async loadData(): Promise<void> {
    const data = game.settings.get(this.storageKey, 'bankData') as any;
